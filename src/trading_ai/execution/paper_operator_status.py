...
def build_paper_operator_status(
    *,
    ...
    if cycle is None:
        blockers.append(_blocker("WARNING", "cycle_missing", "latest paper auto cycle is missing", source_path=cycle_path))
        latest_cycle = None
    else:
    ...
    if monitor is not None:
        ...
        if monitor_payload is None:
            blockers.append(_blocker("ERROR", "monitor_invalid", "monitor artifact is missing or invalid", source_path=monitor))
        else:
    ...
    if performance is not None:
        ...
        if performance_payload is None:
            blockers.append(_blocker("ERROR", "performance_invalid", "performance artifact is missing or invalid", source_path=performance))
        else:
            ...
            unreconciled_fills = _int_value(statement_summary.get("unreconciled_fills"), default=_int_value(statement.get("missing_fills"), default=0))
...
def _cycle_blockers(path: Path, payload: Mapping[str, object]) -> list[dict[str, object]]:
    ...
    if state == "ERROR":
        ...
        for reason in reason_codes(payload.get("reasons")) or ["cycle_blocked"]:
            blockers.append(_blocker("CRITICAL", reason, f"latest paper auto cycle is blocked: {reason}", source_path=path))
    elif state == "PAPER_SUBMITTED":
        blockers.append(_blocker("CRITICAL", "paper_cycle_pending_closeout", "previous paper cycle submitted and needs closeout evidence", source_path=path))
...
def _ledger_summary(path: Path) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    for line_number, raw_line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        try:
            ...
            blockers.append(_blocker("ERROR", "session_ledger_invalid_json", f"invalid ledger JSON at line {line_number}: {exc}", source_path=path))
            continue
    ...
    clean_sessions = [
        ...
        if str(record.get("state") or "").upper() in {"PAPER_SUBMITTED", "PAPER_CLOSED"} and not reason_codes(record.get("blockers"))
    ]
    blocked_sessions = [
        record
        for record in records
...
def _monitor_blockers(path: str | Path, payload: Mapping[str, object]) -> list[dict[str, object]]:
    ...
    if _int_value(counts.get("orders"), default=0) > 0:
        blockers.append(_blocker("CRITICAL", "open_broker_orders", "paper broker snapshot has open orders", source_path=path))
    if _int_value(counts.get("positions"), default=0) > 0:
        blockers.append(_blocker("CRITICAL", "existing_positions", "paper broker snapshot has positions", source_path=path))
    for alert in _object_list(payload.get("alerts")):
        if not isinstance(alert, Mapping):
            continue
        severity = str(alert.get("severity") or "").upper()
        if severity in {"CRITICAL", "ERROR"}:
            blockers.append(_blocker(severity, str(alert.get("code") or "monitor_alert"), str(alert.get("message") or alert.get("code") or "monitor alert"), source_path=path))
    blockers.extend(_safety_blockers(path, payload))
    return blockers
...
def _performance_blockers(path: str | Path, payload: Mapping[str, object]) -> list[dict[str, object]]:
    ...
    if _int_value(metrics.get("pending_closeouts"), default=0) > 0:
        blockers.append(_blocker("CRITICAL", "closeout_pending", "paper performance has pending closeouts", source_path=path))
    if _int_value(metrics.get("unmatched_closeouts"), default=0) > 0:
        blockers.append(_blocker("CRITICAL", "closeout_unmatched", "paper performance has unmatched closeouts", source_path=path))
    ...
    if statement_status in {"ERROR", "MISMATCH", "UNMATCHED", "DIFFERENCES", "MISSING"}:
        blockers.append(_blocker("CRITICAL", "statement_mismatch", "paper statement reconciliation is not matched", source_path=path))
    statement_summary = _mapping(payload.get("statement_status"))
    if str(statement_summary.get("status") or "").upper() == "STATEMENT_PENDING":
        blockers.append(_blocker("CRITICAL", "statement_pending", "paper statement is pending for local fills", source_path=path))
    if _int_value(statement.get("missing_fills"), default=0) > 0:
        blockers.append(_blocker("CRITICAL", "fills_unreconciled", "paper statement has unreconciled fills", source_path=path))
...
def _artifact_summary(path: str | Path, payload: Mapping[str, object]) -> dict[str, object]:
    return {"path": str(Path(path)), "status": str(payload.get("status") or "UNKNOWN"), "generated_at": str(payload.get("generated_at") or "")}
...
def _safety_blockers(path: str | Path, payload: Mapping[str, object]) -> list[dict[str, object]]:
    ...
    if safety.get("broker_client_built") is True:
        blockers.append(_blocker("CRITICAL", "broker_client_built", "broker client was built before operator status", source_path=path))
    if safety.get("credentials_read") is True:
        blockers.append(_blocker("CRITICAL", "credentials_read", "credentials were read before operator status", source_path=path))
    if safety.get("orders_submitted") is True:
        blockers.append(_blocker("CRITICAL", "orders_submitted", "orders were submitted before operator status", source_path=path))
    if safety.get("live_trading_authorized") is True or safety.get("live_trading_allowed") is True:
        blockers.append(_blocker("CRITICAL", "live_trading_not_allowed", "live trading must remain disabled", source_path=path))
    return blockers
