...
class AlpacaPaperBroker:
    ...
    def read_account(self) -> PaperAccount:
        ...
        return PaperAccount(
            ...
            status=str(_get_attr(account, "status", "")),
    ...
    def list_orders(self, *, status: str = "open") -> tuple[PaperOrderSnapshot, ...]:
        if self._dry_run or self._client is None:
            return ()
        raw_orders = self._client.get_orders(filter=_build_get_orders_request(status))
    ...
    def submit_order(self, order: PaperOrder) -> PaperOrderResult:
        ...
        response = _submit_market_order(self._client, symbol=symbol, order=order)
...
def _paper_order_snapshot_from_raw(order: Any) -> PaperOrderSnapshot:
    return PaperOrderSnapshot(
        ...
        side=str(_get_attr(order, "side", "")),
        order_type=str(_get_attr(order, "order_type", _get_attr(order, "type", ""))),
        time_in_force=str(_get_attr(order, "time_in_force", "")),
        status=str(_get_attr(order, "status", "")),
